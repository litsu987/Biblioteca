...
def crear_usuarios(num_usuarios):
    for _ in range(num_usuarios):
        ...
        usuari = Usuari.objects.create(
            ...
            imatge=fake.image_url(),  # Añadir imagen aleatoria
            centre=Centre.objects.order_by('?').first(),  # Asignar un centro aleatorio
            cicle=Cicle.objects.order_by('?').first()  # Asignar un ciclo aleatorio
        )
